...
CHORD_BEGIN
	STATE(NODE(identifier), TOKEN(col), NODE(identifier), TOKEN(col), TOKEN(eq), NODE(expression))
	...
	REDUCE_TO(variable_decl, name, type_name, initialiser);
	return {.t = result::type::reduce_success};
CHORD_END
...
CHORD_BEGIN
	...
	SETINDEX(3);
	syntax::node::expression initialiser = GETNODE(expression);
	REDUCE_TO(variable_decl, name, syntax::node::identifier{syntax::node::inferred_typename}, syntax::node::expression{});
	return {.t = result::type::reduce_success};
CHORD_END
