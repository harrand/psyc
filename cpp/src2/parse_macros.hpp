...
#define STATE(...) [](){return std::array{__VA_ARGS__};}(), [](reducer reduce)->result{std::size_t index = 0; constexpr std::size_t count = VARARGS_COUNT(__VA_ARGS__);diag::assert_that(reduce.subtrees.size() + 1 >= count, error_code::ice, "current parser state ({} subtrees excluding +1 lookahead token) is too small for this chord, which demands at least {} subtrees + lookaheads total", reduce.subtrees.size(), count);
